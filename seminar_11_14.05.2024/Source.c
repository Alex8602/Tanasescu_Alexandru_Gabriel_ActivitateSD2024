...
Nod* inserareInArbore(Nod* arbore, Rezervare rez) {
	if (arbore != NULL) {
		...
		int grad = calculGradDeEchilibru(arbore);
		if (grad == 2) {
...
int initializareArbore(Nod* rad) {
	if (rad) {
		int initializareStanga = initializareArbore(rad->stanga);
		int initializareDreapta = initializareArbore(rad->dreapta);
		//int maxim;
		/*if (initializareStanga > initializareDreapta) {
			maxim = initializareStanga;
		}
		else { maxim = initializareDreapta; }
		maxim = maxim + 1;
		return maxim;*/
		return 1 + (initializareStanga > initializareDreapta ? initializareStanga : initializareDreapta);
	}
	else { return 0; }
}
...
int calculGradDeEchilibru(Nod* rad) {
	if (rad) {
		int initializareStanga = initializareArbore(rad->stanga);
		int initializareDreapta = initializareArbore(rad->dreapta);
		return initializareStanga - initializareDreapta;
	}
}
